...
const generateTradesForDate = (date: string): Trade => {
  const symbols = ['NIFTY', 'BANKNIFTY', 'RELIANCE', 'TCS', 'INFY', 'HDFC', 'ICICI', 'SBIN'];
...
const currentBrokerTrades = computed(() => {
  ...
  return tradeData.value[brokerKey]?.map(trade => ({
    title: trade.title,
    date: trade.date,
    backgroundColor: trade.pnl >= 0 ? 'rgba(34, 197, 94, 0.1)' : 'rgba(239, 68, 68, 0.1)',
...
const stats = computed(() => {
  ...
  return {
    totalTrades,
    profitableTrades,
    winRate: totalTrades > 0 ? ((profitableTrades / totalTrades) * 100).toFixed(1) : '0.0',
    totalPnL
  };
});
</script>
    ...
    <div class="calendar-header">
      ...
      <div class="stats-container">
        ...
        <div class="stat-card">
          ...
          <div class="stat-value" :class="{ 'text-green': stats.totalPnL > 0, 'text-red': stats.totalPnL < 0 }">
